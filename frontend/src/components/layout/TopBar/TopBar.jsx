...
function TopBar() {
  ...
  const { pendingRequests, friendAcceptedToast } = useFriends();
  ...
  // 🔵 Friend accepted toast — driven by FriendsContext
  const [showFriendToast, setShowFriendToast] = useState(false);
  const [friendToastName, setFriendToastName] = useState("");
  const [isToastClosing, setIsToastClosing] = useState(false);

  useEffect(() => {
    if (friendAcceptedToast) {
      setFriendToastName(
        friendAcceptedToast.firstName || friendAcceptedToast.username,
      );
  ...
  // Handle toast close with fade-out animation
  ...
  return (
    <>
              ...
              >
                ...
                {(pendingRequests.length > 0 || unreadCount > 0) && (
                  <span className="notification-badge">
                    {pendingRequests.length + unreadCount}
                  </span>
                )}
              </div>
              <div
      ...
      {/* Message Modal */}
      {isMessageModalOpen && <MessageModal onClose={closeMessages} />}
      ...
      {showFriendToast && (
        <div
          className={`friend-accepted-toast ${isToastClosing ? "closing" : ""}`}
        >
          <FriendsIcon size={18} />
          <span>
            <strong>{friendToastName}</strong> accepted your friend request!
          </span>
      ...
      {newMessageNotification && (
        <div className="friend-accepted-toast" onClick={() => openMessages()}>
          ...
          <span>
            <strong>{newMessageNotification.senderUsername}</strong> sent you a
            message
          </span>
        </div>
      )}
